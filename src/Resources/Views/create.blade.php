...
@section('card-title', "Create new task")

@section('content')
    <form method="POST" action="{{ route('cron-bundle.store') }}">
        {{ csrf_field() }}
        @if(isset($errors))
        ...
        <div class="form-group">
            ...
            <input type="text" name="name" id="create-name" class="form-control" placeholder="Name of the new task"
                   value="{{ old("name") }}" aria-describedby="nameHelp" autocomplete="off">
                ...
                <select name="command" id="create-command" class="form-control text-monospace">
                    ...
                    @foreach(\Tolacika\CronBundle\CronBundle::getAvailableCommands() as $command)
                        <option value="{{ $command }}"
                                @if (old('command') == $command) selected @endif>{{ $command }}</option>
                    @endforeach
                </select>
                <div class="input-group-addon">
                    <span class="input-group-text">Arguments:</span>
                </div>
                <input type="text" name="args" class="form-control text-monospace" value="{{ old("args") }}"
                       autocomplete="off">
            </div>
        ...
        <div class="form-group">
            ...
            <input type="text" name="schedule" id="create-schedule" class="form-control"
                   ...
                   value="{{ old("schedule") }}" aria-describedby="scheduleHelp" autocomplete="off">
            ...
            <select name="schedule-select" id="create-schedule-select" class="form-control">
                ...
                @foreach(\Tolacika\CronBundle\CronBundle::getPredefinedSchedules() as $sch => $text)
                    <option value="{{ $sch }}" @if($sch == old('schedule')) selected @endif>{{ $text }} ({{ $sch }})
                    </option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
            ...
            <textarea name="description" id="create-desc" class="form-control">{{ old("description") }}</textarea>
        </div>
        <div class="form-group">
            <div class="custom-control custom-switch">
                <input type="checkbox" class="custom-control-input" name="enabled" id="create-enabled"
                       @if (old("enabled")) checked @endif value="true">
